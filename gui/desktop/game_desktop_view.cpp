...
//   std::cout << "Lib name called: " << lib_name << std::endl;
...
void GameDesktopView::startTimer(const char *lib_name) {
  ...
  userInput = (UserInputFunc)dlsym(handle, "userInput");
  if (!userInput) {
    std::cerr << "dlsym userInput error: " << dlerror() << std::endl;
  }

  updateCurrentState = (UpdateCurrentStateFunc)dlsym(handle, "updateCurrentState");
  if (!updateCurrentState) {
    std::cerr << "dlsym updateCurrentState error: " << dlerror() << std::endl;
  }
